...
if compgen -G "${PROJECT_DIR}/tensorrt_llm/bindings.*.so" > /dev/null 2>&1; then
    ...
    # Without compiled bindings every binding type resolves to Any.  The
    # flags below suppress the strict-mode checks that cascade from this:
